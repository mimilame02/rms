      ...
      if(validate_tenants($_POST)){
        ...
        if ($tenant_obj->tenant_fetch($_GET['id'])){
          ...
          $tenant_obj->occupants_relations = json_decode($data['occupants_relations'], true);
            ...
            if (isset($tenant_obj->type_of_household)) {
              $type_of_household = $tenant_obj->type_of_household;
              echo '<script>
                  document.addEventListener("DOMContentLoaded", function() {';

              if ($type_of_household != 'one person') {
                  echo 'document.getElementById("other_occupants_fields").style.display = "block";';
              } else {
                  echo 'document.getElementById("other_occupants_fields").style.display = "none";';
              }
