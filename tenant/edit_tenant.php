        ...
        if ($tenant_obj->tenant_fetch($_GET['id'])){
          ...
          $tenant_obj->occupants_relations = json_decode($data['occupants_relations'], true);
            ...
            if (isset($tenant_obj->relationship_status)) {
              $relationship_status = $tenant_obj->relationship_status;
              echo '<script>
                  document.addEventListener("DOMContentLoaded", function() {';
...
<body>
                  ...
                  <div class="col-md-6">
                    <div class="form-group-row">
                      ...
                      <div class="col">
                        ...
                        <?php
                        if (is_array($tenant_obj->has_vehicle)) {
                            $tenant_has_vehicle = $tenant_obj->has_vehicle;
                        } else {
                        ...
                        $vehicle_types = [
                            ...
                            ['name' => 'others', 'label' => 'Other']
                        ];
                        foreach ($vehicle_types as $vehicle_type) {
                            ?>
                            <input type="checkbox" name="has_vehicle[]" id="<?php echo $vehicle_type['name']; ?>" value="<?php echo $vehicle_type['name']; ?>" <?php if (in_array($vehicle_type['name'], $tenant_has_vehicle)) echo 'checked'; ?>>
                            <label for="<?php echo $vehicle_type['name']; ?>"><?php echo $vehicle_type['label']; ?></label><br>
                        <?php } ?>
                        <div class="d-flex col-sm-12">
                            <label for="vehicle_specification" hidden>If other, please specify:</label><br>
                            <input class="form-control form-control-sm" type="text" name="vehicle_specification" id="vehicle_specification" style="display:none;" value="<?php echo $tenant_obj->vehicle_specification; ?>"><br>
                        </div>
                      </div>
                    </div>
                  </div>
                              ...
                              <div class="col-md-6">
                                  <div class="form-group-row">
                                      <div class="col">
                                          ...
                                          <input type="text" class="form-control form-control-sm" id="occupants" name="occupants[]" value="<?php echo $occupants[$i]; ?>">
                                      </div>
                                  </div>
                              </div>
                              <div class="col-md-6">
                                  <div class="form-group-row">
                                      <div class="col">
                                          ...
                                          <input class="form-control form-control-sm" type="text" id="occupants_relations" name="occupants_relations[]" value="<?php echo $occupants_relations[$i]; ?>">
                                      </div>
                                  </div>
                              </div>
                          </div>
  ...
  <script>
      ...
      document.addEventListener("DOMContentLoaded", function() {
        var type_of_household = '<?php echo isset($tenant_obj->type_of_household) ? $tenant_obj->type_of_household : ''; ?>';
        if (type_of_household != 'one person') {
            console.log('Displaying other occupants fields');
            document.getElementById("other_occupants_fields").style.display = "block";
        } else {
            console.log('Hiding other occupants fields');
            document.getElementById("other_occupants_fields").style.display = "none";
        }
      });
