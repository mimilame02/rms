...
function validate_full_name($POST) {
 ...
 function validate_image($POST) {
  // Check if the input file exists and is a valid upload
  if(empty($POST['identification_document'])){
    return false;
  }
  return true;
}
...
/* end here tenants validation */


function validate_add_landlord($post) {
  if (!validate_first_name($post) || !validate_middle_name($post) || !validate_last_name($post) || !validate_email($post) || !validate_contact_num($post) || !validate_date_birth($post) || !validate_address($post) || !validate_region($post) || !validate_prov($post) || !validate_city($post) ||
  !validate_image($post) || !validate_full_name($post) || !validate_econtact_no($post)){
    return false;
  }
  return true;
}
