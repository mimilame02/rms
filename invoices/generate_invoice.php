...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'fetch_lease_data' && isset($_POST['lease_unit_id'])) {
      ...
      if ($invoice->fixed_bills == 1) {
          ...
          $num_of_invoices = isset($_POST['num_of_invoices']) ? (int)$_POST['num_of_invoices'] : 1;
          $interval_in_months = isset($_POST['interval_in_months']) ? (int)$_POST['interval_in_months'] : 1;
          ...
          // Add a single invoice when the fixed_bills checkbox is not checked
          if (!$invoice->invoice_add()) {
              // Handle invoice add error
              $msg = "Error uploading invoice";
          }
