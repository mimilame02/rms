...
class Properties{
    ...
    function properties_add() {
        ...
        $property_id = $this->db->connect()->lastInsertId();
    
        // Decode the features JSON string into an array of feature IDs
        $features_array = json_decode($this->features, true);
    
        // Insert the features into the property_features table
        foreach ($features_array as $feature_id) {
            $sql = "INSERT INTO property_features (property_id, feature_id) VALUES (:property_id, :feature_id)";
            $query = $this->db->connect()->prepare($sql);
            $query->bindParam(':property_id', $property_id);
            $query->bindParam(':feature_id', $feature_id);
            $query->execute();
        }
